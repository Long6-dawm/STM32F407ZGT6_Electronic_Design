 ...
 * @version     V1.2
 * @date        2024-12-14
 * @brief       lcd_ex.c存放各种LCD驱动IC的寄存器初始化序列执行函数,以及lcd.c,头文件等
...
void lcd_ex_nt35510_reginit(void)
{
    ...
    /* AVDD 5.2V */
    lcd_write_reg(0xB000, 0x0D); /* 0C,调小AVDD电压输出 */
    ...
    /* AVEE -5.2V */
    lcd_write_reg(0xB100, 0x0D); /* 0C调小AVEE电压输出 */
...
void lcd_ex_ssd1963_reginit(void)
{
    ...
    delay_ms(12);
    lcd_wr_regno(0x01); /* 复位 */
    delay_ms(10);

    lcd_wr_regno(0xE6); /* 设置像素时钟频率,33Mhz */
    lcd_wr_data(0x2F);
    lcd_wr_data(0xFF);
    lcd_wr_data(0xFF);
    ...
    lcd_wr_data((SSD_HOR_RESOLUTION - 1) >> 8); /* 设置LCD水平参数 */
    lcd_wr_data(SSD_HOR_RESOLUTION - 1);
    lcd_wr_data((SSD_VER_RESOLUTION - 1) >> 8); /* 设置LCD垂直参数 */
    lcd_wr_data(SSD_VER_RESOLUTION - 1);
    lcd_wr_data(0x00);  /* RGB顺序 */
    ...
    lcd_wr_regno(0xF0); /* 设置SSD1963的CPU接口为16bit */
    lcd_wr_data(0x03);  /* 16-bit(565 format) data for 16bpp */

    lcd_wr_regno(0x29); /* 开启显示 */
    /* 打开PWM配置  通过通道占空比可控背光 */
    lcd_wr_regno(0xD0); /* 打开自动空闲亮度DBC */
    lcd_wr_data(0x00);  /* disable */

    lcd_wr_regno(0xBE); /* 打开PWM配置 */
    lcd_wr_data(0x05);  /* 1,设置PWM频率 */
    lcd_wr_data(0xFE);  /* 2,设置PWM占空比 */
    lcd_wr_data(0x01);  /* 3,设置C */
    lcd_wr_data(0x00);  /* 4,设置D */
    lcd_wr_data(0x00);  /* 5,设置E */
    lcd_wr_data(0x00);  /* 6,设置F */

    lcd_wr_regno(0xB8); /* 设置GPIO配置 */
    lcd_wr_data(0x03);  /* 2个IO口设置成输出 */
    lcd_wr_data(0x01);  /* GPIO使用前面IO口输出 */
    lcd_wr_regno(0xBA);
    lcd_wr_data(0X01);  /* GPIO[1:0]=01,控制LCD背光 */
}
